...
/// Shows the classic MRZ scanner component using the native SwiftUI view `SBSDKScannerView`.
struct MRZScannerClassicView: View {
    
    @Binding var isScanningEnabled: Bool
    
    @State private var model: SBSDKMRZScannerViewModel?
    ...
    private func createScannerIfNeeded() {
        ...
        do {
            let model = try SBSDKMRZScannerViewModel(scannerConfiguration: SBSDKMRZScannerConfiguration())
    ...
    private func handle(event: SBSDKFrameEngineEvent<SBSDKMRZScannerResult, SBSDKImageRef, SBSDKImageRef>) {
        switch event {
        case .validResult(let result, _):
            guard result.success else { return }
            state.present(ScanResult(title: "MRZ",
